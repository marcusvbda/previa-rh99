import './_styles.scss';
import DefaultCard from '@/components/cards/defaultCard';

export default function Shortcuts(): JSX.Element {
    return (
        <DefaultCard className="content-card shortcuts">
            <a href="#" className="shortcut">
                <svg
                    xmlns="http://www.w3.org/2000/svg"
                    width="25"
                    height="24"
                    viewBox="0 0 25 24"
                    fill="none"
                >
                    <g clip-path="url(#clip0_794_4302)">
                        <path
                            d="M23.213 4.077C22.9317 3.73944 22.5796 3.46795 22.1815 3.28182C21.7835 3.09568 21.3494 2.99946 20.91 3H4.742L4.7 2.649C4.61405 1.91942 4.26338 1.24673 3.71449 0.758478C3.1656 0.270223 2.45663 0.000341793 1.722 0L1.5 0C1.23478 0 0.98043 0.105357 0.792893 0.292893C0.605357 0.48043 0.5 0.734784 0.5 1C0.5 1.26522 0.605357 1.51957 0.792893 1.70711C0.98043 1.89464 1.23478 2 1.5 2H1.722C1.96693 2.00003 2.20334 2.08996 2.38637 2.25272C2.56941 2.41547 2.68634 2.63975 2.715 2.883L4.091 14.583C4.23385 15.7998 4.81848 16.9218 5.73391 17.736C6.64934 18.5502 7.83185 19 9.057 19H19.5C19.7652 19 20.0196 18.8946 20.2071 18.7071C20.3946 18.5196 20.5 18.2652 20.5 18C20.5 17.7348 20.3946 17.4804 20.2071 17.2929C20.0196 17.1054 19.7652 17 19.5 17H9.057C8.43806 16.9983 7.8348 16.8051 7.32994 16.4471C6.82507 16.089 6.44331 15.5835 6.237 15H18.157C19.3293 15.0001 20.4643 14.5882 21.3638 13.8364C22.2633 13.0846 22.87 12.0407 23.078 10.887L23.863 6.533C23.9414 6.10101 23.9237 5.65707 23.8114 5.23264C23.6991 4.80821 23.4948 4.41368 23.213 4.077ZM21.9 6.178L21.114 10.532C20.9891 11.225 20.6245 11.852 20.0839 12.3032C19.5433 12.7544 18.8612 13.0011 18.157 13H5.919L4.978 5H20.91C21.0569 4.99912 21.2022 5.03062 21.3355 5.09226C21.4689 5.15389 21.587 5.24415 21.6815 5.35661C21.776 5.46907 21.8446 5.60097 21.8824 5.74294C21.9201 5.8849 21.9262 6.03344 21.9 6.178Z"
                            fill="#161616"
                        />
                        <path
                            d="M7.5 24.0006C8.60456 24.0006 9.49999 23.1052 9.49999 22.0006C9.49999 20.896 8.60456 20.0006 7.5 20.0006C6.39543 20.0006 5.5 20.896 5.5 22.0006C5.5 23.1052 6.39543 24.0006 7.5 24.0006Z"
                            fill="#161616"
                        />
                        <path
                            d="M17.5 24.0006C18.6046 24.0006 19.5 23.1052 19.5 22.0006C19.5 20.896 18.6046 20.0006 17.5 20.0006C16.3954 20.0006 15.5 20.896 15.5 22.0006C15.5 23.1052 16.3954 24.0006 17.5 24.0006Z"
                            fill="#161616"
                        />
                    </g>
                    <defs>
                        <clipPath id="clip0_794_4302">
                            <rect
                                width="24"
                                height="24"
                                fill="white"
                                transform="translate(0.5)"
                            />
                        </clipPath>
                    </defs>
                </svg>
                Créditos
                <div className="badge">999</div>
            </a>
            <a href="#" className="shortcut">
                HG - Entrar
            </a>
            <a href="#" className="shortcut">
                RX - Entrar
            </a>
        </DefaultCard>
    );
}
